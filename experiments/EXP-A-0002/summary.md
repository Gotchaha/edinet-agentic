...
> **Erratum (2026-03-30):** The original version incorrectly described the
> sample as "dev eval set from EVAL-0001." The actual sample was the first
> 12 examples by dataset order from the N=50 frozen sample (via `--limit 12`),
> not the curated dev eval set. Baseline comparison metrics have been
> corrected to use the same 12 examples.
...
|--------|-------------------------|----------------------------|
| Accuracy  | 0.417 | 0.667 |
| Precision | 0.000 | 0.636 |
| Recall    | 0.000 | 1.000 |
| F1        | 0.000 | 0.778 |
...
- Run on first 12 examples from N=50 sample (via `--limit 12`), not the
  curated dev eval set from EVAL-0001. Stopped after design flaws identified.
