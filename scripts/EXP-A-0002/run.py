"""Run EXP-A-0002 tool-augmented verification agent.

Usage:
    uv run python scripts/EXP-A-0002/run.py --config configs/EXP-A-0002.yaml
    uv run python scripts/EXP-A-0002/run.py --config configs/EXP-A-0002.yaml --limit 1
"""
...
def _serialize_message(msg) -> dict:
    ...
    if isinstance(content, list):
        # Content blocks from tool-use messages — extract text parts
        content = " ".join(
...
def run_one(graph, example: dict, base_prompt: str, sheets: list[str]) -> dict:
    ...
    # If reviser didn't parse, try parsing from messages as fallback
    ...
    trace = {
        "doc_id": example["doc_id"],
        "messages": [
            _serialize_message(msg) for msg in final_state["messages"]
        ],
    }
